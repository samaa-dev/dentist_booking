...
class BookingState with _$BookingState {
  ...
  const factory BookingState.errorAddBooking(
    String message,
  ) = ErrorAddBooking;

  const factory BookingState.errorUpdateBooking(
    String message,
  ) = ErrorUpdateBooking;

  const factory BookingState.errorDeleteBooking(
    String message,
  ) = ErrorDeleteBooking;

  const factory BookingState.successAddBooking(
    BookingModel booking,
  ) = SuccessAddBooking;
  const factory BookingState.successUpdateBooking(
    BookingModel booking,
  ) = SuccessUpdateBooking;
  const factory BookingState.successDeleteBooking(
    BookingModel? booking,
  ) = SuccessDeleteBooking;

  const factory BookingState.loadingPatients() = _LoadingPatients;
  const factory BookingState.loadedPatients(
    List<ProfileModel> patients,
  ) = _BookingLoadedUser;
  const factory BookingState.errorPatients(
    String message,
  ) = _BookingErrorUser;
}
