-- Update queue_status to show the current booking (being served now) instead of the next one.
-- Changes from original:
--   1. Added: early return when current_queue_number = 0 (not started yet)
--   2. Changed: booking query from queue_number > current to queue_number = current (no status filter)
--   3. Added: separate check for has_more to still detect NO_MORE_BOOKINGS
--   4. Changed: NO_MORE_BOOKINGS now includes current_booking so the last patient stays visible
--   5. Renamed: next_booking → current_booking in the RETURN
...
BEGIN
  ...
  -- [CHANGE 1] لم يبدأ الطابور بعد: إرجاع NORMAL بدون booking ليُفعّل isNotStarted في الـ UI
  ...
  -- [CHANGE 3] التحقق من وجود مواعيد إضافية لمعرفة إذا يجب تعطيل زر "التالي"
  IF NOT EXISTS (
    ...
    -- [CHANGE 4] NO_MORE_BOOKINGS مع current_booking حتى يظل المريض الأخير مرئياً
    RETURN jsonb_build_object(
      'state',
        CASE current_shift
          WHEN 'morning' THEN 'NO_MORE_BOOKINGS_MORNING'
  ...
  -- [CHANGE 5] تغيير next_booking إلى current_booking
  RETURN jsonb_build_object(
    'state','NORMAL',
    'shift',current_shift,
    'current_queue_number', qs.current_queue_number,
