-- Queue: only confirmed bookings count for start/next/status.
-- Fixes starting queue when only cancelled/completed rows exist, and returns
-- NO_MORE_BOOKINGS_* when current_queue_number = 0 and no confirmed bookings.
