-- Minimum accepted app versions per client (run in Supabase SQL Editor)
...
-- Public RPC: readable before auth (anon). Fail-open on clients if call fails.
...
begin
  perform update_clinic_settings(
    ...
    -- Only overwrite min versions when the payload includes at least one key
