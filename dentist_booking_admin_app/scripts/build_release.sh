...
echo "Building debug APK (v${VERSION_FULL}) from ${ENV_FILE} ..."
...
APK_SRC="$ROOT_DIR/build/app/outputs/flutter-apk/app-debug.apk"
...
mkdir -p "$DIST_DIR"
DIST_APK="$DIST_DIR/dentist_booking_admin_app_v${VERSION_FULL}-debug.apk"
cp "$APK_SRC" "$DIST_APK"
...
echo ""
echo "Also available at Flutter output:"
echo "  $APK_SRC"
