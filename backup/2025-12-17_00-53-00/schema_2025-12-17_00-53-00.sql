...
CREATE OR REPLACE FUNCTION "public"."can_book"("p_booking_datetime" timestamp without time zone, "p_shift" "public"."booking_shift", "p_patient_type" "public"."patient_type", "p_patient_id" "uuid" DEFAULT NULL::"uuid") RETURNS "jsonb"
    ...
    AS $$
DECLARE
  v_booking_date date := DATE(p_booking_datetime);  -- تغيير الاسم هنا
...
BEGIN
  ...
  ---------------------------------------------------------------------
  iso := EXTRACT(isodow FROM v_booking_date);  -- استخدام v_booking_date
  dow := CASE 
           WHEN iso = 6 THEN 1
           WHEN iso = 7 THEN 2
           ELSE iso + 2
  ...
  IF p_shift = 'morning' THEN
    ...
    IF v_booking_time < allowed_start THEN  -- استخدام v_booking_time
      RETURN jsonb_build_object(
        'can_book', false, 
    ...
    IF v_booking_time > allowed_end THEN  -- استخدام v_booking_time
      RETURN jsonb_build_object(
        'can_book', false, 
  ...
  ---------------------------------------------------------------------
  -- 8) Capacity check - إصلاح الغموض هنا
  ---------------------------------------------------------------------
  SELECT COUNT(*)
  INTO existing_count
  FROM bookings b  -- تحديد الجدول بوضوح
  WHERE DATE(b.booking_date) = v_booking_date  -- استخدام v_booking_date بدلاً من booking_date
    AND b.shift = p_shift
  ...
  ---------------------------------------------------------------------
  -- 9) Registered patient cannot book twice per day - إصلاح الغموض هنا
  ---------------------------------------------------------------------
  IF p_patient_type = 'registered' THEN
    ...
    WHERE b.patient_id = p_patient_id
      AND DATE(b.booking_date) = v_booking_date  -- استخدام v_booking_date بدلاً من booking_date
      AND b.booking_status IN ('pending','confirmed');
...
CREATE OR REPLACE FUNCTION "public"."cancel_booking"("p_booking_id" "uuid", "p_cancel_reason" "text") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
BEGIN
  ...
  IF v_role = 'staff' THEN
      v_status := 'cancelledByStaff';
  ELSE
      v_status := 'cancelledByPatient';
  END IF;
...
ALTER FUNCTION "public"."cancel_booking"("p_booking_id" "uuid", "p_cancel_reason" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."create_booking_app"("p_booking_date" timestamp without time zone, "p_shift" "public"."booking_shift", "p_patient_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  ...
  result_data JSONB;
  booking_day DATE := DATE(p_booking_date);
  check_result JSONB; -- إضافة تعريف المتغير هنا
BEGIN
  -- 1) Get auto_confirm setting
  SELECT auto_confirm INTO auto_confirm_enabled
  FROM clinic_settings LIMIT 1;

  -- 2) Set final status based on auto_confirm
  final_status := CASE 
                    WHEN auto_confirm_enabled THEN 'confirmed'
                    ELSE 'pending'
                  END;

  -- 3) Check if booking is allowed
  check_result := can_book(p_booking_date, p_shift, 'registered', p_patient_id); -- تخزين النتيجة
...
ALTER FUNCTION "public"."create_booking_app"("p_booking_date" timestamp without time zone, "p_shift" "public"."booking_shift", "p_patient_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_active_announcements"("platform_param" "text" DEFAULT 'appOnly'::"text") RETURNS TABLE("id" "uuid", "title" "text", "body" "text", "images" "jsonb", "audience" "text", "is_active" boolean, "created_by_user_id" "uuid", "created_at" timestamp with time zone, "updated_at" timestamp with time zone, "link_url" "text", "has_images" boolean, "image_count" integer)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  SELECT 
    ...
    a.updated_at,
    a.link_url,
    (jsonb_array_length(a.images) > 0) as has_images,
    jsonb_array_length(a.images) as image_count
  FROM announcements a
  WHERE a.is_active = true
    AND (
      a.audience = 'all'::announcement_audience
      OR 
      (
        -- تحقق أولاً أن platform_param قيمة صالحة للـ ENUM
        platform_param IN ('all', 'appOnly', 'tvOnly')
        AND a.audience = platform_param::announcement_audience
      )
    )
...
ALTER FUNCTION "public"."get_active_announcements"("platform_param" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_active_announcements_json"("platform_param" "text" DEFAULT 'appOnly'::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  result JSONB;
BEGIN
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'body', a.body,
      'images', a.images,
      'audience', a.audience::TEXT,
      'is_active', a.is_active,
      'created_by_user_id', a.created_by_user_id,
      'created_at', a.created_at,
      'updated_at', a.updated_at,
      'link_url', a.link_url,
      'has_images', jsonb_array_length(a.images) > 0,
      'image_count', jsonb_array_length(a.images),
      'first_image', CASE 
        WHEN jsonb_array_length(a.images) > 0 THEN a.images->>0 
        ELSE NULL 
      END
    )
    ORDER BY a.created_at DESC
  ...
  -- إذا لم توجد نتائج، نرجع مصفوفة فارغة
  RETURN COALESCE(result, '[]'::JSONB);
END;
$$;


ALTER FUNCTION "public"."get_active_announcements_json"("platform_param" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_active_announcements_v2"("user_role_param" "text" DEFAULT NULL::"text", "platform_param" "text" DEFAULT 'app'::"text") RETURNS TABLE("id" "uuid", "title" "text", "body" "text", "images" "jsonb", "audience" "text", "is_active" boolean, "created_by_user_id" "uuid", "created_at" timestamp with time zone, "updated_at" timestamp with time zone, "link_url" "text", "has_images" boolean, "image_count" integer)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  ...
  WHERE a.is_active = true
    AND (
      ...
      a.audience = 'all'
      OR 
      -- إعلانات خاصة بالمنصة
      a.audience = platform_param
    )
  ORDER BY 
    -- إعطاء الأولوية للإعلانات الحديثة
    a.created_at DESC;
END;
$$;


ALTER FUNCTION "public"."get_active_announcements_v2"("user_role_param" "text", "platform_param" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_all_bookings_with_filters_app"("p_user_id" "uuid" DEFAULT NULL::"uuid", "p_start_date" "date" DEFAULT NULL::"date", "p_end_date" "date" DEFAULT NULL::"date", "p_specific_date" "date" DEFAULT NULL::"date", "p_status" "public"."booking_status" DEFAULT NULL::"public"."booking_status", "p_shift" "public"."booking_shift" DEFAULT NULL::"public"."booking_shift", "p_search_query" "text" DEFAULT NULL::"text") RETURNS TABLE("id" "uuid", "booking_date" timestamp with time zone, "patient_id" "uuid", "patient_type" "public"."patient_type", "patient_name" "text", "patient_phone" "text", "patient_address" "text", "shift" "public"."booking_shift", "booking_status" "public"."booking_status", "booking_created_by" "uuid", "queue_number" "text", "ticket_code" "text", "people_before" integer, "created_at" timestamp with time zone, "updated_at" timestamp with time zone, "cancelled_at" timestamp with time zone, "completed_at" timestamp with time zone, "cancelled_by" "uuid", "cancel_reason" "text")
    LANGUAGE "sql"
    AS $$
WITH filtered_bookings AS (
  SELECT *
  ...
  WHERE
    ...
    -----------------------------------------
    -- 📌 فلترة البحث — ❌ بدون guest
    -----------------------------------------
    AND (
      p_search_query IS NULL OR p_search_query = '' OR (
        
        -- البحث فقط للمستخدمين المسجّلين
        (
        ...
        OR b.ticket_code ILIKE '%' || p_search_query || '%'
      )
    )
),
...
ALTER FUNCTION "public"."get_all_bookings_with_filters_app"("p_user_id" "uuid", "p_start_date" "date", "p_end_date" "date", "p_specific_date" "date", "p_status" "public"."booking_status", "p_shift" "public"."booking_shift", "p_search_query" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_app_ads"() RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  result JSONB;
BEGIN
  ...
  FROM announcements a
  WHERE a.is_active = true
    AND a.audience IN ('all'::announcement_audience, 'appOnly'::announcement_audience);
...
ALTER FUNCTION "public"."get_app_ads"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_booking_status"("p_datetime" timestamp without time zone) RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  w RECORD;
...
BEGIN
  -- Convert day of week
  dow := CASE 
           WHEN iso = 6 THEN 1  -- السبت
           WHEN iso = 7 THEN 2  -- الأحد
           ELSE iso + 2         -- الاثنين -> الجمعة
         END;
  ...
  IF w IS NULL OR NOT w.is_open OR NOT cs.is_booking_enabled THEN
    RETURN jsonb_build_object(
      'is_open', false,
      'shift', null,
      'time_left', null,
      'is_booking_enabled', COALESCE(cs.is_booking_enabled, false)
    );
  END IF;

  -- Determine current shift considering allowed time before closing
  IF now_time BETWEEN cs.booking_morning_start_time  AND (w.morning_end - INTERVAL '1 minute' * cs.allow_before_minutes) THEN
    shift := 'morning';
    end_time := w.morning_end - INTERVAL '1 minute' * cs.allow_before_minutes;
  ELSIF now_time BETWEEN w.evening_start AND (w.evening_end - INTERVAL '1 minute' * cs.allow_before_minutes) THEN
    shift := 'evening';
...
ALTER FUNCTION "public"."get_booking_status"("p_datetime" timestamp without time zone) OWNER TO "postgres";
...
BEGIN
    ...
    -- جلب بيانات profiles إذا كان المريض مسجلاً
    ...
    -- تحديد بيانات المريض مع الأولوية لبيانات profiles
    ...
    -- حساب total_in_queue بشكل صحيح
    SELECT COUNT(*) INTO total_in_queue
    FROM bookings b 
    WHERE b.shift = booking_record.shift 
    AND b.booking_date::date = booking_record.booking_date::date
    AND b.booking_status IN ('pending', 'confirmed');

    -- جلب وإحصائيات الطابور
    SELECT 
        qs.current_queue_number,
        qs.is_paused,
        qs.updated_at as last_updated,
        GREATEST(0, booking_record.queue_number::INT - qs.current_queue_number) as patients_before_you,
        total_in_queue as total_in_queue,
        GREATEST(0, booking_record.queue_number::INT - qs.current_queue_number) * 15 as estimated_wait_time
    INTO queue_stats_record
    FROM queue_state qs
    WHERE qs.shift = booking_record.shift;

    -- بناء النتيجة لتتناسب مع TrackingModel
    result := jsonb_build_object(
        'booking', jsonb_build_object(
            ...
            'people_before', queue_stats_record.patients_before_you
        ),
        'queue_stats', jsonb_build_object(
            'current_queue_number', queue_stats_record.current_queue_number,
            'is_paused', queue_stats_record.is_paused,
            'last_updated', queue_stats_record.last_updated,
            'patients_before_you', queue_stats_record.patients_before_you,
            'total_in_queue', queue_stats_record.total_in_queue,
            'estimated_wait_time', queue_stats_record.estimated_wait_time
        )
    );
...
ALTER FUNCTION "public"."get_booking_tracking"("ticket_code_param" "text") OWNER TO "postgres";
...
CREATE INDEX "idx_bookings_booking_date" ON "public"."bookings" USING "btree" ("date"("booking_date"));
...
CREATE INDEX "idx_bookings_shift" ON "public"."bookings" USING "btree" ("shift");
...
CREATE POLICY "Clinic settings readable only by admin" ON "public"."clinic_settings" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "Prevent inserting new working hours rows" ON "public"."working_hours" FOR SELECT TO "authenticated" USING (true);
...
GRANT ALL ON FUNCTION "public"."cancel_booking"("p_booking_id" "uuid", "p_cancel_reason" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."cancel_booking"("p_booking_id" "uuid", "p_cancel_reason" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."cancel_booking"("p_booking_id" "uuid", "p_cancel_reason" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_booking_app"("p_booking_date" timestamp without time zone, "p_shift" "public"."booking_shift", "p_patient_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."create_booking_app"("p_booking_date" timestamp without time zone, "p_shift" "public"."booking_shift", "p_patient_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_booking_app"("p_booking_date" timestamp without time zone, "p_shift" "public"."booking_shift", "p_patient_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_active_announcements"("platform_param" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_active_announcements"("platform_param" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_active_announcements"("platform_param" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_active_announcements_json"("platform_param" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_active_announcements_json"("platform_param" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_active_announcements_json"("platform_param" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_active_announcements_v2"("user_role_param" "text", "platform_param" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_active_announcements_v2"("user_role_param" "text", "platform_param" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_active_announcements_v2"("user_role_param" "text", "platform_param" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_all_bookings_with_filters_app"("p_user_id" "uuid", "p_start_date" "date", "p_end_date" "date", "p_specific_date" "date", "p_status" "public"."booking_status", "p_shift" "public"."booking_shift", "p_search_query" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_all_bookings_with_filters_app"("p_user_id" "uuid", "p_start_date" "date", "p_end_date" "date", "p_specific_date" "date", "p_status" "public"."booking_status", "p_shift" "public"."booking_shift", "p_search_query" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_all_bookings_with_filters_app"("p_user_id" "uuid", "p_start_date" "date", "p_end_date" "date", "p_specific_date" "date", "p_status" "public"."booking_status", "p_shift" "public"."booking_shift", "p_search_query" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_app_ads"() TO "anon";
GRANT ALL ON FUNCTION "public"."get_app_ads"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_app_ads"() TO "service_role";



GRANT ALL ON FUNCTION "public"."get_booking_status"("p_datetime" timestamp without time zone) TO "anon";
GRANT ALL ON FUNCTION "public"."get_booking_status"("p_datetime" timestamp without time zone) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_booking_status"("p_datetime" timestamp without time zone) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_booking_tracking"("ticket_code_param" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."get_booking_tracking"("ticket_code_param" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_booking_tracking"("ticket_code_param" "text") TO "service_role";
